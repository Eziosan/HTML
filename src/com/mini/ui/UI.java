...
//			String singer = sc.next();
	...
	public void concertInfo() {
		...
		while (flag) {
			...
			switch(select) {
				case 1 : concertInfo_singer(); break; // 가수명으로 콘서트 검색
				case 2 : concertAllInfo(); break;// 콘서트의 모든 정보를 출력
	...
	public void concertInfo_singer() {
		// 가수명에 해당하는 콘서트 정보를 불러 오면 됨.
		System.out.println("검색할 가수를 입력해 주세요 : ");
		
		System.out.println("11매니저 추가되는 대로 추가할거");
		
	}
	
	
	
	public void concertAllInfo() {
		// 그냥 모든 콘서트 정보를 가져 오면 됨.
		
		System.out.println("22매니저 추가되는 대로 추가할거.");
	}
